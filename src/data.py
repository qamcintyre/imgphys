...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        train_dataset = NBodyDataset(wandb_config=self.wandb_config, train=True)
        val_dataset = NBodyDataset(wandb_config=self.wandb_config, train=False)
...
class NBodyDataset(torch.utils.data.Dataset):
    def __init__(self, wandb_config: Dict[str, Any], train: bool):
        ...
        self.data = torch.tensor(
            np.load(wandb_config["data_path"]), dtype=torch.float32
        )

        if train:
            self.data = self.data[
                : int(wandb_config["train_val_split"] * self.data.shape[0])
            ]
        else:
            self.data = self.data[
                int(wandb_config["train_val_split"] * self.data.shape[0]) :
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            if "gaussian_noise" in self.wandb_config["dataset_kwargs"]["transforms"]:
                datum = self.add_gaussian_noise(
                    datum, eps=self.wandb_config["dataset_kwargs"]["eps"]
                )
        return datum[:, :-1], datum[:, -1]

    def add_gaussian_noise(self, datum: Tensor, eps=1) -> Tensor:
        """Add Gaussian noise to the input data.
        Args:
            datum: Tensor of shape (1, T, C, H, W).
        Returns:
            Tensor of shape (1, T, C, H, W) with added Gaussian noise.
        """
        noise = torch.randn_like(datum) * eps
        datum = datum + noise
        datum = torch.clamp(datum, 0, 1)
        return datum
